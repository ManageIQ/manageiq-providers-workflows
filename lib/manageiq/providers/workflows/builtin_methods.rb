    ...
    module Workflows
      class BuiltinMethods < BasicObject
        ...
        def self.provision_execute(_params = {}, _secrets = {}, context = {})
          ...
          return ::ManageIQ::Providers::Workflows::BuiltinRunnner.error!({}, :cause => "Calling provision_execute on non-provisioning request: [#{object_type}]") unless object_type == "ServiceTemplateProvisionTask"
          return ::ManageIQ::Providers::Workflows::BuiltinRunnner.error!({}, :cause => "Missing MiqRequestTask id") if object_id.nil?

          miq_request_task = ::MiqRequestTask.find_by(:id => object_id.to_i)
          return ::ManageIQ::Providers::Workflows::BuiltinRunnner.error!({}, :cause => "Unable to find MiqReqeustTask id: [#{object_id}]") if miq_request_task.nil?
        ...
        private_class_method def self.miq_task_status!(runner_context)
          ...
          return ::ManageIQ::Providers::Workflows::BuiltinRunnner.error!(runner_context, :cause => "Unable to find MiqTask id: [#{runner_context["miq_task_id"]}]") if miq_task.nil?
          ...
          unless runner_context["running"]
            ...
            if runner_context["success"]
              ...
              ::ManageIQ::Providers::Workflows::BuiltinRunnner.error!(runner_context, :cause => miq_task.message)
            end
          end
        ...
        private_class_method def self.miq_request_task_status!(runner_context)
          ...
          when nil
            ...
            ::ManageIQ::Providers::Workflows::BuiltinRunnner.error!(runner_context, :cause => reason)
          when "error"
          ...
          when "retry"
            ...
            runner_context
          when "ok"
            ::ManageIQ::Providers::Workflows::BuiltinRunnner.success!(runner_context, :output => {"Result" => "provisioned"})
          end
        end
      end
    end
